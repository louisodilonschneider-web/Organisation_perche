...
regiePerche/
  ...
  dons/<id>           → une personne/entreprise sollicitée pour le mécénat
```
...
- **Meeting — 6ème** : liste de chaque classe avec, par élève, la feuille d'information rendue ou non, la réponse au meeting, le nombre de places souhaitées, le transport, le nombre de places attribuées et leur numéro. Le transport propose 5 choix : Personnel / Besoin de covoiturage (avec qui) / Ne sait pas comment venir — compte sur un covoiturage (pour les familles qui veulent venir mais n'ont pas encore de solution) / Propose du covoiturage (nombre de places offertes + élèves emmenés) / Autre. Les lignes sont colorées (blanc/bleu/jaune/vert) selon que l'élève ne demande rien, attend une attribution, a une attribution partielle ou complète.
- **Sortie & meeting — 3ème** : suivi individuel (autorisation de sortie, argent reçu, souhait de meeting, places, transport — mêmes 5 choix que pour les 6ème).
- **Invités hors-classe** : attribution de places à des collègues ou contributeurs au projet, en dehors des classes.
- **Dons & mécénat** : ajoutez chaque entreprise, association ou personne démarchée pour le mécénat (bouton "+ Ajouter une entreprise démarchée" — nom + type). Pour chacune : case "Papier remis" (dossier de mécénat remis), puis le retour (sans réponse / oui / non). Dès qu'un retour "Oui" est enregistré, le montant du don et une éventuelle attribution de places au meeting (s'il en reste sur l'enveloppe globale) deviennent modifiables. Ces places attribuées sont comptées dans le total de la capacité meeting, comme pour les 6ème, 3ème et invités.
